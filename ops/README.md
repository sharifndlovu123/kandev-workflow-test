...
| `workflow-diagram.py` | Live vertical workflow sequence with selectable tasks, labeled route summaries, a chronological task timeline with explicit returns, and task details. Local only, stdlib only, read-only DB access. `./ops/workflow-diagram.py [--port 8420]`, then open `http://localhost:8420/`. See `docs/specs/live-workflow-diagram.md`. Also runs as an always-on `systemd --user` service (`kandev-workflow-diagram.service`, ordered `After=kandev.service`) so it's already up whenever a Kandev task starts — `systemctl --user status/restart kandev-workflow-diagram`. |
